...
function Dashboard({ moderator, onLogout }) {
  ...
  const handleAddCheck = async (userId) => {
    if (!window.confirm('Chek qo\'shishni tasdiqlaysizmi?')) return;
    try {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-indigo-50 p-5">
      <div className="max-w-7xl mx-auto space-y-6">
        <header className="bg-white rounded-2xl p-5 flex justify-between items-center shadow-sm border border-slate-100 animate-fade-in">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              onClick={onLogout}
              className="px-4 py-2.5 bg-slate-100 text-slate-600 rounded-xl hover:bg-red-50 hover:text-red-600 transition-all duration-300 font-medium"
            >
              🚪 Chiqish
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-blue-200 transition-all">
            <div className="flex items-center gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-purple-200 transition-all">
            <div className="flex items-center gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 hover:border-orange-200 transition-all">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-slate-800">
                  {stats.total > 0 ? (stats.checks / stats.total).toFixed(1) : 0}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 animate-fade-in" style={{ animationDelay: '0.2s' }}>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 animate-fade-in" style={{ animationDelay: '0.3s' }}>
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-slate-100">
      ...
      {showQR && selectedUser && (
        <QRModal user={selectedUser} onClose={() => setShowQR(false)} />
      )}
      {showHistory && selectedUser && (
        <HistoryModal user={selectedUser} onClose={() => setShowHistory(false)} />
      )}
    </div>
  );
}
